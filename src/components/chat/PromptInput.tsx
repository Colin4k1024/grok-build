...
export function PromptInput({ onSend, onCancel, isStreaming, disabled, cwd, onSwitchProject }: Props) {
  ...
  const { isRecording, interimText, toggleRecording, stopRecording, language, setLanguage } = useVoiceInput((transcript, isFinal) => {
    setText(prev => isFinal ? prev + transcript : prev.replace(/\s*\[…\]\s*$/, "") + transcript);
  });
  ...
  const handleSend = () => {
    ...
    setHistoryIdx(-1); setText(""); setShowSlash(false); clearImages();
  };
    ...
    <div className="relative px-4 pb-3 pt-2" onDrop={onDrop} onDragOver={onDragOver}>
      ...
      {isRecording && (
        <div className="mb-2 flex items-center gap-2 rounded-md bg-gb-red/10 px-2.5 py-1">
          ...
          <select value={language} onChange={e => setLanguage(e.target.value as "auto"|"zh-CN"|"en-US")} className="rounded bg-transparent px-1 text-[10px] text-gb-muted outline-none">
            <option value="auto">Auto</option><option value="zh-CN">中文</option><option value="en-US">EN</option>
          </select>
        </div>
      )}
      ...
      <div className="flex items-end gap-2 rounded-lg border border-gb-border/10 bg-gb-surface/50 px-3 py-2 focus-within:border-gb-accent/40">
        <button className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-md transition-colors ${isRecording ? "text-gb-red" : "text-gb-muted hover:text-gb-text"}`} onClick={toggleRecording} disabled={disabled}>
        ...
        <textarea ref={textareaRef} className="flex-1 resize-none bg-transparent text-[13px] text-gb-text outline-none placeholder:text-gb-muted" rows={1} placeholder="Send a message…" value={text} onChange={e => { setText(e.target.value); setHistoryIdx(-1); }} onKeyDown={handleKeyDown} disabled={disabled} />
        {isStreaming ? (
          <button className="shrink-0 rounded-md bg-gb-red/15 px-3 py-1.5 text-[12px] font-medium text-gb-red hover:bg-gb-red/25" onClick={onCancel}>Stop</button>
        ) : (
          <button className="shrink-0 rounded-md bg-gb-accent px-3.5 py-1.5 text-[12px] font-medium text-white hover:opacity-80 disabled:opacity-20" onClick={handleSend} disabled={(!text.trim() && images.length === 0) || disabled}>
