...
`/flush`, `/dream`, and `/memory` require memory to be enabled (`--memory` or `GROK_MEMORY=1`); `/memory` also needs a configured memory backend. `/remember` is always available.
