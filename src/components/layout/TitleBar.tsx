...
export function TitleBar({ auth, config, onLogout, onNewSession, creating, onToggleSidebar, onToggleRightPanel }: TitleBarProps) {
  ...
  const handleModelChange = useCallback(async (modelId: string) => {
    ...
    setModelOpen(false);
    try { await setSessionModel(activeSessionId, modelId); } catch (e) { console.error("Failed to set model:", e); }
  ...
  return (
    <header className="flex h-11 shrink-0 items-center justify-between border-b border-gb-border/8 px-3">
      <div className="flex items-center gap-2">
        <button className="rounded p-1 text-gb-muted hover:bg-gb-surface-hover hover:text-gb-text" onClick={onToggleSidebar}>
        ...
        <div ref={modelRef} className="relative">
          <button className="flex items-center gap-1 rounded px-2 py-1 text-[12px] text-gb-text-secondary hover:bg-gb-surface-hover hover:text-gb-text disabled:opacity-30" onClick={() => setModelOpen(v => !v)} disabled={!activeSessionId}>
          ...
          {modelOpen && (
            <div className="absolute left-0 top-full z-50 mt-1 max-h-60 w-56 overflow-y-auto rounded-md border border-gb-border/10 bg-gb-surface-solid py-0.5 shadow-lg">
              {config?.models.filter(m => !m.hidden).map(m => (
                <button key={m.id} className={`flex w-full items-center justify-between px-2.5 py-1.5 text-left text-[12px] hover:bg-gb-surface-hover ${m.id === currentModel ? "text-gb-accent" : "text-gb-text"}`} onClick={() => handleModelChange(m.id)}>
        ...
        <div ref={effortRef} className="relative">
          <button className="flex items-center gap-1 rounded px-2 py-1 text-[12px] text-gb-text-secondary hover:bg-gb-surface-hover hover:text-gb-text disabled:opacity-30" onClick={() => setEffortOpen(v => !v)} disabled={!activeSessionId}>
          ...
          {effortOpen && (
            <div className="absolute left-0 top-full z-50 mt-1 w-32 rounded-md border border-gb-border/10 bg-gb-surface-solid py-0.5 shadow-lg">
              {efforts.map(e => (
                <button key={e.id} className={`flex w-full items-center justify-between px-2.5 py-1.5 text-left text-[12px] hover:bg-gb-surface-hover ${e.id === currentEffort ? "text-gb-accent" : "text-gb-text"}`} onClick={() => handleEffortChange(e.id)}>
                  <span>{e.label}</span>
      ...
      <div className="flex items-center gap-1.5">
        <button className="flex items-center gap-1 rounded px-2.5 py-1 text-[12px] font-medium text-gb-text hover:bg-gb-surface-hover disabled:opacity-30" onClick={onNewSession} disabled={creating}>
          {creating ? "…" : "New"}
        </button>
        <button className="rounded p-1 text-gb-muted hover:bg-gb-surface-hover hover:text-gb-text" onClick={onToggleRightPanel}>
        ...
        <div ref={menuRef} className="relative">
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full z-50 mt-1 w-48 rounded-md border border-gb-border/10 bg-gb-surface-solid py-0.5 shadow-lg">
              ...
              <button className="w-full px-2.5 py-1.5 text-left text-[12px] text-gb-red hover:bg-gb-surface-hover" onClick={() => { setMenuOpen(false); onLogout(); }}>Sign out</button>
            </div>
          )}
        </div>
      </div>
