...
fn selected_evolution(
    ...
    let tracked_ids = agent.tracked_session_ids();
    if let Some(session_id) = session_id {
        let handle = tracked_ids
            .iter()
            .find(|id| id.to_string() == session_id)
    ...
    if tracked_ids.len() == 1 {
        let id = &tracked_ids[0];
        return Ok(agent.resident_handle(id).map(|handle| SelectedEvolution {
            service_slot: handle.evolution_service.clone(),
            workspace: PathBuf::from(&handle.info.cwd),
            cmd_tx: handle.cmd_tx.clone(),
        }));
