...
export function PromptInput({ onSend, onCancel, isStreaming, disabled, cwd, onSwitchProject }: Props) {
  ...
  const handleSend = () => {
    ...
    console.log("[PromptInput] handleSend called", { trimmed, images: images.length });
  ...
  return (
    <div className="relative px-4 pb-3 pt-2" onDrop={onDrop} onDragOver={onDragOver}>
      ...
      <div className="flex items-end gap-2 rounded-lg border border-gb-border/10 bg-gb-surface/50 px-3 py-2 focus-within:border-gb-accent/40">
        ...
        <textarea ref={textareaRef} className="flex-1 resize-none bg-transparent text-[13px] text-gb-text outline-none placeholder:text-gb-muted" rows={1} placeholder="Send a message… (Enter to send)" value={text} onChange={e => { setText(e.target.value); setHistoryIdx(-1); }} onKeyDown={handleKeyDown} disabled={disabled} />
        {isStreaming ? (
      ...
      {(cwd !== undefined || onSwitchProject) && (
        <div className="mt-1.5 flex items-center justify-between px-1">
          ...
          <span className="text-[10px] text-gb-muted">⏎ send · ⇧⏎ newline</span>
        </div>
      )}
    </div>
  );
