    ...
    async fn evolution_model_completion(
        ...
        let request = ConversationRequest {
            items: vec![
                ...
                ConversationItem::user(user.to_owned()),
            ],
            model: Some(model),
            max_output_tokens: Some(max_tokens),
            x_grok_conv_id: Some(format!("evolution-{}", uuid::Uuid::new_v4())),
            x_grok_req_id: Some(format!("xai-evolution-{}", uuid::Uuid::new_v4())),
            x_grok_session_id: Some(self.session_info.id.to_string()),
            x_grok_agent_id: Some(xai_grok_telemetry::id::agent_id()),
            ..Default::default()
        };
        let future = sampling_client.conversation_collect(request);
